...
## 2024-07-08 - [Lazy Directory Evaluation]
**Learning:** Using `Directory.GetFiles` with `SearchOption.AllDirectories` for traversing whole drives or large directories causes massive upfront array memory allocations and fails with `UnauthorizedAccessException` on protected system folders.
**Action:** Always prefer `Directory.EnumerateFiles` with `new EnumerationOptions { IgnoreInaccessible = true, RecurseSubdirectories = true }` for lazy evaluation and resilience against access exceptions.
