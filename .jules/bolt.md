...
## 2024-07-07 - [Optimize File Traversal Memory Overhead]
**Learning:** Using `Directory.GetFiles` with `SearchOption.AllDirectories` on whole drives allocates massive string arrays and throws `UnauthorizedAccessException` on system folders. `Directory.EnumerateFiles` with `new EnumerationOptions { IgnoreInaccessible = true, RecurseSubdirectories = true }` enables lazy evaluation and avoids access crashes.
**Action:** Always prefer `Directory.EnumerateFiles` with `EnumerationOptions` for large directory traversal.
