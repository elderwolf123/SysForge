...
/// <summary>
/// Gzip compression encoder using .NET's built-in Gzip implementation.
/// Good general-purpose compression with wide compatibility.
/// </summary>
public class GzipEncoder : IHyperEncoder
{
    ...
    {
        // Gzip provides good compression ratios
        if (IsTextLike(sample))
            return 0.35f; // Text compresses well
        else if (IsRepetitive(sample))
            return 0.30f; // Repetitive data compresses well
        else
    ...
    {
        // Gzip is suitable for most data types except already compressed files
