...
/// <summary>
/// LZMA compression encoder using .NET's built-in LZMA implementation.
/// Provides excellent compression ratios for various data types.
/// </summary>
public class LZMAEncoder : IHyperEncoder
{
    ...
    {
        // LZMA is not directly available in .NET Core/5+ without additional packages
        // For now, fall back to Gzip which provides good compression ratios
    ...
    {
        // LZMA typically provides excellent compression ratios
        if (IsTextLike(sample))
            return 0.20f; // Text compresses extremely well
        else if (IsRepetitive(sample))
            return 0.15f; // Repetitive data compresses excellently
        else
            return 0.40f; // Binary average
    }
