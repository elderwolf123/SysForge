...
public class PerformanceModeManager
{
    ...
    private void SetWindowsPowerPlan(PerformanceMode mode)
    {
        try
        {
            ...
            // Security: Use absolute path for powercfg.exe to prevent path hijacking and LPE
            string powercfgPath = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "powercfg.exe");
