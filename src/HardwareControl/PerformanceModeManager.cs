...
public class PerformanceModeManager
{
    ...
    private void SetWindowsPowerPlan(PerformanceMode mode)
    {
        try
        {
            ...
            // Security: Use absolute path for powercfg to prevent path hijacking
            var powercfgPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "powercfg.exe");
