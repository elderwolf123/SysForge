...
namespace RamOptimizer.ProcessManagement
{
    public class ProcessRecoveryEngine
    {
        ...
        {
            // ⚡ BOLT OPTIMIZATION:
            // What: Initialize dictionary with StringComparer.OrdinalIgnoreCase.
            // Why: Avoids unnecessary string allocations and GC pressure from calling .ToLower() during lookups.
            // Impact: O(1) lookups without heap allocations.
    ...
    public class RestartProcessStrategy : RecoveryStrategy
    {
        public override async Task RecoverAsync(CancellationToken cancellationToken)
        {
            try
            {
                // SECURITY: Use absolute path for explorer to prevent Path Hijacking vulnerabilities
