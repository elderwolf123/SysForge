    ...
    public class SystemStabilityTester : IStabilityTester
    {
        ...
        private bool CheckCriticalProcesses()
        {
            try
            {
                ...
                // Remove .exe extensions since GetProcessesByName and ProcessName don't include it
                ...
                // ⚡ BOLT OPTIMIZATION: Fetch processes once (O(N)) instead of calling GetProcessesByName in a loop (O(N*M))
                var runningProcessNames = new HashSet<string>(Process.GetProcesses().Select(p => p.ProcessName), StringComparer.OrdinalIgnoreCase);
